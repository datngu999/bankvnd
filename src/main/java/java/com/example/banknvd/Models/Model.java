package com.example.banknvd.Models;

import com.example.banknvd.Views.ViewFactory;
...
public class Model {
        ...
        try {
            if (resultSet != null && resultSet.next()) {
                ...
                LocalDate localDate = LocalDate.of(
                        Integer.parseInt(dateTrans[0]),
                        Integer.parseInt(dateTrans[1]),
                        Integer.parseInt(dateTrans[2])
                );
                ...
                System.out.println("Không tìm thấy tài khoản với địa chỉ và mật khẩu đã nhập.");
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public void prepareTransactions(ObservableList<Transaction> transactions, int limit) {
        ...
        try {
            while (resultSet.next()) {
                ...
                LocalDate dateTrans = resultSet.getDate("Date").toLocalDate(); // Không cần formatter
    ...
    public void setAllTransactions() {
        prepareTransactions(this.allTransactions, 0); // 0 nghĩa là không giới hạn
    }
    ...
    public ObservableList<Client> searchClient(String payeeAddress) {
        ...
        try {
            if (resultSet.next()) { // ✅ Phải gọi trước khi truy xuất dữ liệu
                String fName = resultSet.getString("firstName");
                String lName = resultSet.getString("lastName");
                ...
                System.out.println("Không tìm thấy tài khoản với địa chỉ: " + payeeAddress);
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
